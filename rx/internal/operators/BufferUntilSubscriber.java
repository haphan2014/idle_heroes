...
public final class BufferUntilSubscriber<T> extends Subject<T, T> {
    static final Observer EMPTY_OBSERVER = new C20531();
    private boolean forward;
    final State<T> state;

    static class C20531 implements Observer {
        C20531() {
        }
    ...
    static final class OnSubscribeAction<T> implements OnSubscribe<T> {
        ...
        class C20541 implements Action0 {
            C20541() {
            }
        ...
        public void call(Subscriber<? super T> s) {
            if (this.state.casObserverRef(null, s)) {
                s.add(Subscriptions.create(new C20541()));
                boolean win = false;
