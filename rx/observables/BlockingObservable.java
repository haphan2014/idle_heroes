...
public final class BlockingObservable<T> {
    ...
    private final Observable<? extends T> f3842o;

    class C22572 implements Iterable<T> {
        C22572() {
        }
    ...
    class C22638 implements Action1<Throwable> {
        C22638() {
        }
    ...
    public void forEach(final Action1<? super T> onNext) {
        ...
        BlockingUtils.awaitForComplete(latch, this.f3842o.subscribe(new Subscriber<T>() {
    ...
    public Iterator<T> getIterator() {
        return BlockingOperatorToIterator.toIterator(this.f3842o);
    }

    public T first() {
        return blockForSingle(this.f3842o.first());
    }

    public T first(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f3842o.first(predicate));
    }

    public T firstOrDefault(T defaultValue) {
        return blockForSingle(this.f3842o.map(UtilityFunctions.identity()).firstOrDefault(defaultValue));
    }

    public T firstOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f3842o.filter(predicate).map(UtilityFunctions.identity()).firstOrDefault(defaultValue));
    }

    public T last() {
        return blockForSingle(this.f3842o.last());
    }

    public T last(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f3842o.last(predicate));
    }

    public T lastOrDefault(T defaultValue) {
        return blockForSingle(this.f3842o.map(UtilityFunctions.identity()).lastOrDefault(defaultValue));
    }

    public T lastOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f3842o.filter(predicate).map(UtilityFunctions.identity()).lastOrDefault(defaultValue));
    }

    public Iterable<T> mostRecent(T initialValue) {
        return BlockingOperatorMostRecent.mostRecent(this.f3842o, initialValue);
    }

    public Iterable<T> next() {
        return BlockingOperatorNext.next(this.f3842o);
    }

    public Iterable<T> latest() {
        return BlockingOperatorLatest.latest(this.f3842o);
    }

    public T single() {
        return blockForSingle(this.f3842o.single());
    }

    public T single(Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f3842o.single(predicate));
    }

    public T singleOrDefault(T defaultValue) {
        return blockForSingle(this.f3842o.map(UtilityFunctions.identity()).singleOrDefault(defaultValue));
    }

    public T singleOrDefault(T defaultValue, Func1<? super T, Boolean> predicate) {
        return blockForSingle(this.f3842o.filter(predicate).map(UtilityFunctions.identity()).singleOrDefault(defaultValue));
    }

    public Future<T> toFuture() {
        return BlockingOperatorToFuture.toFuture(this.f3842o);
    }
    ...
    public void subscribe() {
        ...
        BlockingUtils.awaitForComplete(cdl, this.f3842o.subscribe(new Subscriber<T>() {
    ...
    public void subscribe(Observer<? super T> observer) {
        ...
        Subscription s = this.f3842o.subscribe(new Subscriber<T>() {
            public void onNext(T t) {
    ...
    public void subscribe(Action1<? super T> onNext) {
        subscribe(onNext, new C22638(), Actions.empty());
    }
