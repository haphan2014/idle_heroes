    ...
    public static synchronized String id(WeakReference<Context> context) {
        ...
        synchronized (C0195g.class) {
            if (context.get() == null) {
                ...
                if (sID == null) {
                    str = C0195g.readInstallationSP(context);
                    if (str != null) {
                        sID = str;
                    } else {
                        try {
                            ...
                            if (file.exists()) {
                                sID = C0195g.readInstallationFile(file);
                                file.delete();
                            } else {
                                sID = C0195g.generateId(context);
                            }
                            C0195g.writeInstallationSP(context, sID);
                        } catch (Throwable e) {
                            C0188a.afLogE("Error getting AF unique ID", e);
                        }
                    }
                }
                str = sID;
