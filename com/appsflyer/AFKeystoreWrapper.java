...
class AFKeystoreWrapper {
    ...
    private void initKeyStore() {
        C0188a.afLog("Initialising KeyStore..");
        try {
        ...
        C0188a.afLog("Couldn't load keystore instance of type: AndroidKeyStore");
    }
                ...
                try {
                    ...
                    while (aliases.hasMoreElements()) {
                        ...
                        if (str != null && isAppsFlyerPrefix(str)) {
                            ...
                            if (split.length == 3) {
                                C0188a.afLog("Found a matching AF key with alias:\n" + str);
                                try {
                                    ...
                                    Object th2 = th;
                                    C0188a.afLog("Couldn't list KeyStore Aliases: " + th2.getClass().getName());
                                    z = z2;
                                    return z;
                                }
                            }
    ...
    private void createKey(String alias) {
        C0188a.afLog("Creating a new key with alias: " + alias);
        try {
            synchronized (this.lock) {
                if (this.keystore.containsAlias(alias)) {
                    C0188a.afLog("Alias already exists: " + alias);
                } else {
            ...
            C0188a.afLog("Exception " + e.getMessage() + " occurred");
        }
    }

    private void deleteKey(String alias) {
        C0188a.afLog("Deleting key with alias: " + alias);
        try {
            synchronized (this.lock) {
